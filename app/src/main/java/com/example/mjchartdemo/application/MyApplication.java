package com.example.mjchartdemo.application;
...
import com.example.mjchartdemo.urlconn.EdusLoggerInterceptor;
    ...
    private void initIpPort() {
        try {
            ...
            if (map.size() > 0) {//如果存在储存值
                ...
                if (port.equals("")) {
                    Constant.sysUrl = ip + "/" + project + "/";
                } else {
                    Constant.sysUrl = ip + ":" + port + "/" + project + "/";
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
